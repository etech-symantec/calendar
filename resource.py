...
def run(playwright):
    ...
    try:
        ...
        if table_handle and table_handle.count() > 0:
            ...
            table_body_html = "<tr><td>데이터를 불러오지 못했습니다. (테이블 없음)</td></tr>"

    except Exception as e:
        print(f"[ERROR] Python calculation failed: {e}")
        table_body_html = f"<tr><td>에러 발생: {e}</td></tr>"
    ...
    <head>
        ...
        <style>
            ...
            /* 헤더 스타일 */
            thead th, .header-row th {{ background-color: #e5e7eb !important; font-weight: bold !important; position: sticky; top: 0; z-index: 10; color: #374151; }}
    ...
    <body>
        ...
        <script>
            ...
            function applyFilter(team) {{
                ...
                // header-row 클래스가 없는 행만 가져옴 (헤더 제외)
                const rows = Array.from(document.querySelectorAll('#scheduleTable tbody tr:not(.header-row)'));
                let visibleRows = [];
            ...
            function updateSummary(visibleRows) {{
                ...
                visibleRows.forEach(r => {{
                    ...
                    if(isTodayGroup) {{
                        ...
                        // 일정명 추출 (안전하게)
                        let title = "일정";
                        if(cells.length > 2) title = cells[2].innerText.trim();
                        else if(cells.length > 1) title = cells[1].innerText.trim();
